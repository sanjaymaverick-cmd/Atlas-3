...
async function main() {
  ...
  try {
    ...
    if (!/N\. Bhatia/i.test(session))
      note("P1", "login", "Seat name N. Bhatia not visible after login");
    ...
    if (!/hot/i.test(hub))
      note("P1", "sales-hub", "Hub does not surface hot leads in the 5-second frame");
    if (!/hold/i.test(hub))
      note("P2", "sales-hub", "Hub does not surface holds in the 5-second frame");
    ...
    if (!/QA Review Lead/i.test(pipeAfter))
      note("P0", "pipeline", "Ingested lead not visible after Ingest & score");
    ...
    if (await assignGupta.count()) {
      ...
      if (value) await assignGupta.selectOption(value);
        ...
        await assignGupta.selectOption({ label: "N. Bhatia · in-house" }).catch(async () => {
    ...
    if (!/L\. Bhati/i.test(chHomes)) {
      note(
        "P0",
        "channel",
        "After switching to Aravalli Homes, Desert Reach hold L. Bhati still missing",
      );
    }
    ...
    if (!/Desert Reach/i.test(chHomes) && !/Shekhawat/i.test(chHomes)) {
      note(
        "P1",
        "channel",
        "Desert Reach firm name / agent not labelled on live holds after entity switch",
      );
    }
    ...
    const _reportedGate = /hold is refused until today’s report/i.test(chHomes);
    await page.getByRole("button", { name: /file daily report/i }).click();
    await page.waitForTimeout(500);
    const _afterReport = await page
    ...
    if (!/C-304/i.test(ho))
      note("P1", "handover", "Mansar handover C-304 not visible on Aravalli entity");
    ...
    const person = page
      .getByRole("button", { name: /S\. Bhargava|V\. Agarwal|G\. Singh|L\. Bhati|R\. Soni/i })
      .first();
    if (await person.count()) {
      ...
      const firstCard = page
        .locator("h1 ~ div button, main button")
        .filter({ hasText: /\./ })
        .first();
    ...
    if (/Approve/i.test(appr) && !/View only/i.test(appr)) {
      note(
        "P2",
        "approvals",
        "SM may have Approve on non-sales items (PO/VO) — check intended authority",
      );
    }
    ...
    if (!/Pink City/i.test(firm))
      note("P0", "company", "Pink City Channel not listed for in-house SM");
    if (!/Desert Reach|Shekhawat/i.test(firm))
      note("P0", "company", "Desert Reach / R. Shekhawat not listed for in-house SM");
    ...
    for (const [path, name] of HIDDEN_PATHS) {
      ...
      const url = page.url();
      const blocked =
        /does not post books|view only is not offered|not offered to|this desk does not/i.test(
          text,
        ) ||
      ...
      if (path === "/app/finance" && tallyActions) {
        note(
          "P0",
          "finance",
          "Deep-link /app/finance exposes Tally reconcile / exception actions to SM",
        );
      }
      if (
        path === "/app/finance" &&
        !/does not post books|Tally stays with Finance/i.test(text) &&
        tallyActions
      ) {
      ...
      if (path !== "/app/finance" && !blocked) {
        note(
          "P1",
          `hidden-${name}`,
          `Deep-link ${path} renders the full module instead of a closed gate`,
          text.slice(0, 220),
        );
      }
    }
    ...
    if (!/stale|expir/i.test(hub2)) {
      note(
        "P1",
        "sales-hub",
        "No stale-hold ageing on command (persona: who is sitting on a hold?)",
      );
    }
    ...
    for (const [path, name, needle] of [
      ...
      screens.push({
        ...
        findMs: 0,
        title: stats.title,
        hit: needle.test(stats.textSample),
        stats,
      });
    }
  } catch (err) {

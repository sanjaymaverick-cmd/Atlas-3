...
import { openTrial, signIn, signOut, readStore, closeTrial } from "../session.mjs";
...
const out = await page.evaluate(() => {
  ...
  const rep = s.fileDailyReport({ agentId: me.id, calls: 5, visits: 1, leads: 1, holds: 0,
    bookings: 0, cancellations: 0, notes: "Lock probe day." });
  ...
  return {
    ...
    reholdBooked: booked ? st.holdUnit({ unitId: booked.id, agentId: me.id, customer: "Probe A", until: "2026-09-10" }) : "no booked unit",
    heldUnit: held?.id,
    reholdHeld: held ? window.__atlasStore.getState().holdUnit({ unitId: held.id, agentId: me.id, customer: "Probe B", until: "2026-09-10" }) : "no held unit",
  };
});

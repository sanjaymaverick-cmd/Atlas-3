      ...
      async run(page, api) {
        await api.act("File diary — washout", () =>
          window.__atlasStore.getState().addDiary({
            projectId: "p_kanak",
            date: "2026-09-10",
            weather: "Heavy rain, 27C",
            labour: 46,
            work: "Work stopped 1030. Slab shuttering covered. Dewatering Tower B pit.",
            materials: "Nil.",
            safety: "Ladder access restricted, wet surfaces.",
            deviceKey: "eng-a1-2026-09-10",
          }),
        );
        api.note(
          "ux",
          "No way to mark a day as a washout",
      ...
      async run(page, api) {
        ...
        await api.act("Ingest a 99acres enquiry", () => {
          ...
          return s.ingestLead({
            ...
            source: "99acres",
            unit: "B-0906",
            note: "Portal enquiry, 3BHK, budget stated",
            budget: 11_500_000,
            kind: "flat",
          });
        });
        ...
        api.note(
          ...
          "Scored on ingest",
          fresh
            ? `score=${fresh.score} band=${fresh.band} model=${fresh.scoreModel} reasons=[${(fresh.scoreReasons ?? []).join(", ")}]`
            : "not found",
        );

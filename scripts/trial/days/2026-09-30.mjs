  ...
  seats: [
      ...
      async run(page, api) {
        await api.act("File diary", () =>
          window.__atlasStore.getState().addDiary({
            ...
            materials: "RMC 280 cum. Cement 300 bags.",
            safety: "No incidents. Month-end safety walk with PD.",
    ...
    },
    {
      ...
      async run(page, api) {
        ...
        api.note(
          "did",
          `Obligations not filed: ${due.length}`,
          due
            .map((o) => `${o.kind}:${o.title}:${o.due}`)
            .slice(0, 5)
            .join(" · ") || "none",
        );
    ...
    },
    {
      ...
      async run(page, api) {
        ...
        api.note(
          "did",
          "Progress at Q2 close",
          (projects ?? [])
            .map((p) => `${p.code} ${p.status} ${p.progress}% spent=${p.spent ?? 0}`)
            .join(" · "),
        );
        const changes = await api.read("changes");
        api.note(
          "did",
          `Changes open at close: ${(changes ?? []).filter((c) => c.status !== "closed").length}`,
        );
      },
    },
    {
    ...
    },
    {
      ...
      async run(page, api) {
        ...
        await api.act("Accept a case as an exception", () => {
          const s = window.__atlasStore.getState();
          const c = (s.tally ?? []).find(
            (t) => t.status !== "reconciled" && t.status !== "exception",
          );
    ...
    },
    {
      ...
      async run(page, api) {
        ...
        api.note(
          "did",
          `Pending at Q2 close: ${pend.length}`,

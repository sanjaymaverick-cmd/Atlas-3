...
function CompanyAdmin() {
  const { user, agents, partners, dailyReports, holds, bookings, commissions, inviteAgent, setAgentStatus } = useAtlas();
  ...
  const ids = companyAgentIds(agents, isThirdParty(user?.role) ? companyId : undefined).filter((id) => {
    const a = agents.find((x) => x.id === id);
    return a && !a.inHouse;
  });
  const rows = agents.filter((a) => ids.includes(a.id) && (isThirdParty(user?.role) ? a.companyId === companyId : !a.inHouse));
    ...
    <div>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Kpi
          ...
          value={String(rows.reduce((s, a) => s + agentScorecard(a, { dailyReports, holds, bookings, commissions }).calls, 0))}
        />
        <Kpi label="Live holds" value={String(holds.filter((h) => h.status === "held" && rows.some((a) => a.id === h.agentId)).length)} />
        <Kpi
          label="Commission accrued"
          value={inr(
            commissions.filter((c) => (!scopedCompany || c.partnerId === scopedCompany) && c.status === "accrued").reduce((s, c) => s + c.amount, 0),
            true,
          )}
            ...
            <Card key={a.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
              <div>
                ...
                <p className="text-xs text-muted">
                  {a.phone} · {sc.calls} calls · {sc.visits} visits · {sc.liveHolds} holds · {sc.booked} booked · {partners.find((p) => p.id === a.companyId)?.name}
                </p>
              </div>
              <div className="flex items-center gap-2">
                <Status value={a.status} />
                {a.status === "active" ? (
                  <Button size="sm" variant="outline" onClick={() => toast(setAgentStatus(a.id, "suspended") ?? "Suspended.")}>
                    Suspend
                  </Button>
                ) : (
                  <Button size="sm" variant="outline" onClick={() => toast(setAgentStatus(a.id, "active") ?? "Active.")}>
                    Activate
                  </Button>
                )}
              </div>

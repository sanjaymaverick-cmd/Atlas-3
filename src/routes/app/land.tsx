...
function Land() {
  ...
  const [deed, setDeed] = useState<
    Record<string, { inr: string; no: string; date: string; advocate: string }>
  >({});
  ...
  const addPid =
    scopedParcels[0]?.projectId ?? projects.find((p) => p.entityId === entityId)?.id ?? "";
    ...
    <div>
      ...
      <div className="space-y-4">
        {scopedParcels.map((r) => {
          ...
          const readyToBuy =
            r.status !== "acquired" && items.length > 0 && items.every((i) => i.status === "clear");
          return (
            <Card key={r.id} className="p-5">
                ...
                <div>
                  ...
                  {fund ? (
                    <p className="mt-1 text-xs text-muted">
                      Construction finance: {fund.bank} {fund.loanPct}/{fund.equityPct} ·{" "}
                      {fund.sanctionNo}
                    </p>
                  ) : r.status === "acquired" ? (
                    <p className="mt-1 text-xs text-muted">
              ...
              <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-3">
                <div>
                  ...
                  <dd className="tabular-nums">
                    {obligations.some(
                      (o) =>
                        o.projectId === r.projectId && o.kind === "rera" && o.status === "filed",
                    )
                      ? r.rera
                      : r.rera
                ...
                <div>
                  ...
                  <dd className="tabular-nums">
                    {r.considerationInr ? inr(r.considerationInr, true) : "—"}
                  </dd>
                </div>
                <div>
              ...
              {r.status === "identified" ? (
                <Button
                  ...
                  variant="outline"
                  onClick={() =>
                    toast(startDiligencePack(r.id) ?? "Standard title pack opened (five checks).")
                  }
                >
                        ...
                        {i.status !== "clear" ? (
                          <>
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => setDiligence(i.id, "flagged")}
                            >
                              Flag
                            </Button>
              ...
              {readyToBuy ? (
                <GateBanner>
                  Title pack is clear. Step 1: consideration ₹. Step 2: sale deed number. Step 3:
                  RERA challan before you mark filed.
                </GateBanner>
              ) : null}
              {(() => {
                const reraObs = scopedObs.filter(
                  (o) => o.projectId === r.projectId && o.kind === "rera",
                );
                ...
                const patch = (partial: Partial<typeof d>) =>
                  setDeed((s) => ({
                    ...
                    [r.id]: {
                      inr: s[r.id]?.inr ?? "",
                      no: s[r.id]?.no ?? "",
                      date: s[r.id]?.date ?? todayIso(),
                      advocate: s[r.id]?.advocate ?? "",
                      ...partial,
                    },
                  }));
                return (
                  <div
                    className={`mt-4 ${readyToBuy || r.status === "acquired" ? "rounded-xl border border-primary/40 p-4" : ""}`}
                  >
                    <div className="mb-3 flex flex-wrap gap-2">
                      {(["Consideration ₹", "Sale deed number", "RERA challan"] as const).map(
                        (label, i) => {
                      ...
                      <div className="grid gap-3">
                        <Field label="Consideration (₹)">
                          <Input
                            type="number"
                            value={d.inr}
                            onChange={(e) => patch({ inr: e.target.value })}
                          />
                        </Field>
                        <Button
                    ...
                    {step === 2 ? (
                      <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        <Field label="Advocate (optional)">
                          <Input
                            value={d.advocate}
                            onChange={(e) => patch({ advocate: e.target.value })}
                          />
                        </Field>
                    ...
                    {step === 3 ? (
                      <div className="grid gap-3">
                        ...
                        {reraObs.length === 0 ? (
                          <p className="text-sm text-muted">
                            No RERA obligation on this project yet. Add one below if needed.
                          </p>
                        ) : (
                          reraObs.map((o) => (
                                ...
                                <>
                                  <Field label="Challan / acknowledgement (required)">
                                    <Input
                                      ...
                                      value={ack[o.id] ?? ""}
                                      onChange={(e) =>
                                        setAck((s) => ({ ...s, [o.id]: e.target.value }))
                                      }
                                      aria-required
                                    />
                                  </Field>
        ...
        {scopedObs.map((o) => (
          <Card key={o.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
            ...
            <div className="flex items-center gap-2">
              <Status
                value={
                  o.status === "filed" ? "approved" : o.status === "overdue" ? "fail" : "pending"
                }
              />
              {o.status !== "filed" ? (

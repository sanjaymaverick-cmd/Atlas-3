...
function ProjectGates({ projectId }: { projectId: string }) {
  ...
  const openDd = parcel
    ? diligence.filter((d) => d.parcelId === parcel.id && d.status !== "clear").length
    : 0;
  const reraFiled = obligations.some(
    (o) => o.projectId === projectId && o.kind === "rera" && o.status === "filed",
  );
  ...
  const landGate = !parcel
    ? "No parcel"
    : parcel.status === "acquired"
      ? "Land acquired"
      : openDd
        ? `Land checks open (${openDd})`
        : "Ready to acquire";
  return (
    <Card className="mt-6 grid gap-3 p-5 sm:grid-cols-2 lg:grid-cols-4">
      <div>
        ...
        <p className="text-xs text-muted">
          {parcel?.considerationInr ? inr(parcel.considerationInr, true) : "No ₹ on file"}
        </p>
      </div>
      <div>
        ...
        <p className="text-xs text-muted">
          {reraFiled
            ? (reraOpen.find((o) => o.status === "filed")?.filedRef ?? parcel?.rera)
            : `Target ${parcel?.rera ?? "—"}`}
        </p>
      </div>
      <div>
        ...
        <p className="text-xs text-muted">
          {fund ? `${fund.bank} ${fund.loanPct}/${fund.equityPct}` : "No sanction master"}
        </p>
      </div>
      <div>
    ...
    <div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <Card className="p-5">
          ...
          <Link
            to="/app/drawings"
            className="mt-2 ml-4 inline-block text-xs text-muted underline-offset-4 hover:underline"
          >
